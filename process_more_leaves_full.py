...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python process_moreleaves.py <raw_filename>")
        sys.exit(1)
    ...
    print(f"\n[{get_timestamp()}] Calibration frame statistics:")
    print(f"  Black: Min={black_frame.min()}, Max={black_frame.max()}, Mean={black_frame.mean():.1f}")
    print(f"  White: Min={white_frame.min()}, Max={white_frame.max()}, Mean={white_frame.mean():.1f}")

    print(f"\n[{get_timestamp()}] Applying dark/white calibration")
    ...
    print(f"\n[{get_timestamp()}] Applying spectral correction")
    ...
    ax1 = plt.subplot(2, 3, 1); ax1.imshow(rgb); ax1.set_title('RGB (Spectral Corrected)'); ax1.axis('off')
    ...
    plt.tight_layout()
    plt.savefig(f'MoreLeaves_Complete_{timestamp}.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'MoreLeaves_NDVI_{timestamp}.png', dpi=150, bbox_inches='tight')
    plt.close()
